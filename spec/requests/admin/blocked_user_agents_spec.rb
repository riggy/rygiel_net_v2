...
RSpec.describe "Admin::BlockedUserAgents", type: :request do
  let(:token) { "test-analytics-token" }
  let(:auth_headers) { { "Authorization" => "Bearer #{token}" } }

  before do
    BlockedUserAgent.delete_all
  ...
  describe "GET /admin/blocked_user_agents" do
    it "returns a JSON array of all patterns" do
      BlockedUserAgent.create!(pattern: "masscan")
      BlockedUserAgent.create!(pattern: "zgrab")

      get "/admin/blocked_user_agents", headers: auth_headers
  ...
  describe "POST /admin/blocked_user_agents" do
    it "creates a new blocked UA pattern" do
      ...
      expect(BlockedUserAgent.find_by(pattern: "evilbot/1.0")).not_to be_nil
      expect(Rails.cache.read("blocked_user_agent_patterns")).to be_nil
    end
